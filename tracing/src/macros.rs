...
#[macro_export]
#[doc(hidden)]
macro_rules! event_log_first_with_event_parent {
    (parent: $parent:expr, $lvl:expr, { $($fields:tt)* }) => {
        if $crate::lt_max_level!($lvl) {
            ...
            $crate::if_log_cfg! {{
                let do_tracing_log = !$crate::dispatcher::has_been_set();
                if enabled || do_tracing_log {
                    match $crate::valueset_all!(meta.fields(), $($fields)*) {
                        value_set => {
                            if do_tracing_log {
                                $crate::__macro_support::__tracing_log_macro_replacement($lvl, meta, value_set)
                            }
                ...
                if enabled {
                    $crate::Event::child_of($parent, meta, $crate::valueset_all!(meta.fields(), $($fields)*));
                }
            }}
        }
    }
...
#[macro_export]
#[doc(hidden)]
macro_rules! event_dispatch_first_log_second {
    ($lvl:expr, { $($fields:tt)* }) => {
        if $crate::lt_max_level!($lvl) {
            let meta = __CALLSITE.metadata();
            let enabled = $crate::lt_current_filter!($lvl)
            ...
            $crate::if_log_cfg! {{
                ...
                if enabled {
                    $crate::Event::dispatch(meta, $crate::valueset_all!(meta.fields(), $($fields)*));
                }
            }}
        }
    }
...
macro_rules! event {
    ...
    (name: $name:expr, target: $target:expr, parent: $parent:expr, $lvl:expr, { $($fields:tt)* } )=> ({
        ...
        $crate::event_log_first_with_event_parent!(parent: $parent, $lvl, { $($fields)* })
    });
    ...
    (name: $name:expr, target: $target:expr, $lvl:expr, { $($fields:tt)* } )=> ({
        ...
        $crate::event_dispatch_first_log_second!($lvl, { $($fields)* })
    });
...
macro_rules! level_enabled {
    ($lvl:expr) => {
        $crate::lt_max_level!($lvl) && $crate::lt_current_filter!($lvl)
    };
}
...
macro_rules! valueset_all {
    ...
    (@ { $(,)* $($out:expr),* }, $($k:ident).+ = ?$val:expr, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, ($crate::__macro_support::Option::Some(&debug(&$val) as &dyn Value)) },
            $($rest)*
        )
    };
    (@ { $(,)* $($out:expr),* }, $($k:ident).+ = %$val:expr, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, ($crate::__macro_support::Option::Some(&display(&$val) as &dyn Value)) },
            $($rest)*
        )
    };
    (@ { $(,)* $($out:expr),* }, $($k:ident).+ = $val:expr, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, ($crate::__macro_support::Option::Some(&$val as &dyn Value)) },
            $($rest)*
        )
    };
    (@ { $(,)* $($out:expr),* }, $($k:ident).+, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, ($crate::__macro_support::Option::Some(&$($k).+ as &dyn Value)) },
            $($rest)*
        )
    };
    (@ { $(,)* $($out:expr),* }, ?$($k:ident).+, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, ($crate::__macro_support::Option::Some(&debug(&$($k).+) as &dyn Value)) },
            $($rest)*
        )
    };
    (@ { $(,)* $($out:expr),* }, %$($k:ident).+, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, ($crate::__macro_support::Option::Some(&display(&$($k).+) as &dyn Value)) },
            $($rest)*
        )
    };
    ...
    (@ { $(,)* $($out:expr),* }, { $k:expr } = ?$val:expr, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, (Some(&debug(&$val) as &dyn Value)) },
            $($rest)*
        )
    };
    (@ { $(,)* $($out:expr),* }, { $k:expr } = %$val:expr, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, (Some(&display(&$val) as &dyn Value)) },
            $($rest)*
        )
    };
    (@ { $(,)* $($out:expr),* }, { $k:expr } = $val:expr, $($rest:tt)*) => {
        $crate::valueset_all!(
            @ { $($out),*, (Some(&$val as &dyn Value)) },
            $($rest)*
        )
    };
    ...
    (@ { $(,)* $($out:expr),* }, $($rest:tt)+) => {
        $crate::valueset_all!(@ { ($crate::__macro_support::Option::Some(&$crate::__macro_support::format_args!($($rest)+) as &dyn Value)), $($out),* },)
    };
